...
// Evidence-based POI density engine for legacy route compatibility.
class POIDensityEngine {
  constructor() {
    this.roadPoiCount = new Map();
    this.allPois = [];
    this.loaded = false;
  }
  ...
  async load() {
    ...
    console.log('[POI Engine] Top 10 busy roads:', sorted.map(([r, c]) => `${r}(${c})`).join(', '));
  }
  ...
  _countNearby(lat, lng, radiusKm) {
    ...
    for (const poi of this.allPois) {
      if (this._fastDistance(lat, lng, poi.lat, poi.lng) <= radiusKm) count += 1;
    }
    return count;
  }
  ...
  _normalize(name) {
    if (!name) return '';
    return name.trim()
      .replace(/^(Duong|duong|\u0110\u01b0\u1eddng|\u0111\u01b0\u1eddng)\s+/i, '')
      .toLowerCase()
      .normalize('NFC');
  }
